...
CONFIG_PATH="/app/ui/dist/squadron-config.js"
CONFIG_TEMPLATE_PATH="/app/ui/dist/squadron-config.js.template"
...
if [ -n "$RUNTIME_URL" ] && [ -f "$CONFIG_PATH" ]; then
  ...
  sed -i "s|__SQUADRON_BACKEND_URL__|$ESCAPED_URL|g" "$CONFIG_PATH"
fi
