...
import {
  ...
  updateSavedQueryApi,
  deleteSavedQueryApi,
  type SquadronQLRequest,
  type SquadronQLResponse,
  type QueryTemplate,
  type FunctionInfo,
  type SavedQuery,
  type SavedQueryInput,
} from "../api/squadron-ql";
...
/**
 * Hook for managing Squadron QL queries
 */
export function useSquadronQL() {
  const [query, setQuery] = useState("");
  const [isExecuting, setIsExecuting] = useState(false);
  const [results, setResults] = useState<SquadronQLResponse | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [history, setHistory] = useState<QueryHistoryItem[]>([]);

  // Execute query
  const execute = useCallback(
    async (customQuery?: string, options?: Partial<SquadronQLRequest>) => {
      ...
      try {
        const request: SquadronQLRequest = {
          query: queryToExecute,
          ...options,
        };

        const response = await executeSquadronQL(request);
        setResults(response);
...
export function useSavedQueries() {
  const fetcher = useCallback(async () => {
    ...
    return response.saved_queries;
  }, []);

  const { data, error, isLoading, mutate } = useSWR<SavedQuery[]>(
  ...
  const create = useCallback(
    async (input: SavedQueryInput) => {
      await createSavedQueryApi(input);
      await mutate();
    },
    [mutate],
  );
  ...
  const update = useCallback(
    async (id: string, input: SavedQueryInput) => {
      await updateSavedQueryApi(id, input);
      await mutate();
    },
    [mutate],
  );
