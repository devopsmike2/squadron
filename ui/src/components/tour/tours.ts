...
export const TOURS: Tour[] = [
    ...
    steps: [
      {
        ...
        title: "Instrument my cloud",
        body: "Squadron finds what in your cloud isn't sending telemetry yet, and writes the Terraform to fix it. We've loaded a sample AWS account with real findings so you can see it working. There's nothing to connect or configure. Click Next.",
      },
      {
        route: "/discovery/aws?account=demo-000000000000",
        ...
        body: "Squadron has scanned the sample account and inventoried what's running (EC2, Lambda, RDS), flagging which resources are already instrumented versus which have observability gaps. Open this tab to see it.",
      },
      {
        route: "/discovery/aws?account=demo-000000000000",
...

  {
    ...
    blurb:
      "Edit a collector config once and stage it across the fleet with a canary-first rollout that watches health and rolls back on its own if a stage goes bad.",
    icon: "Rocket",
    duration: "~2 min",
    steps: [
      {
      ...
      },
      {
        ...
        body: "This is every OpenTelemetry Collector config Squadron tracks, all versioned, so editing one creates a new revision you can roll out. Open any config to see the diff-aware editor with live validation.",
      },
      {
        route: "/rollouts",
...

  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "This banner fires only when there's an open, unacknowledged cost spike, meaning projection jumped well above the rolling baseline. It's the heads-up an on-call engineer needs before the invoice arrives.",
      },
      {
        route: "/",
...

  {
    ...
    blurb:
      "Squadron inventories what's actually running and generates Terraform import blocks, bringing drifted, click-built resources under version control as a merge-ready PR.",
    icon: "FileCode",
    duration: "~2 min",
    steps: [
      {
