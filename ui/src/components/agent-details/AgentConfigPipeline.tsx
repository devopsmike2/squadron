...
export function AgentConfigPipeline({
  ...
  const configIntent = agent?.config_intent;
  const driftStatus = agent?.drift_status;
  const driftDetails = agent?.drift_details;
  const intendedConfig = configIntent?.content || "";
  const hasDiff = Boolean(
    driftStatus === "drifted" && intendedConfig && configContent,
  );
  ...
  const driftTitle = (() => {
    switch (driftStatus) {
      ...
      case "no_intent":
        return "No configuration intent";
      case "no_effective":
        return "Effective config missing";
      default:
        return "Configuration state unknown";
    }
  })();
  ...
  const driftDescriptionText = (() => {
    switch (driftStatus) {
      ...
      case "no_intent":
        return "Assign a group or agent config to establish an intent.";
      case "no_effective":
        return "The agent hasn't reported its effective config yet.";
      default:
        return "Waiting on config intent or agent telemetry.";
    }
  })();

  const intentSummary = configIntent
    ? `${
        configIntent.source === "group"
          ? `Group: ${configIntent.source_name || agent?.group_name || "unnamed"}`
          : "Agent override"
      } · v${configIntent.version}`
    : "No assigned intent";

  return (
    <div className="space-y-4">
      <Alert variant={driftStatus === "drifted" ? "destructive" : "default"}>
        <AlertTitle>{driftTitle}</AlertTitle>
        <AlertDescription>{driftDescriptionText}</AlertDescription>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="text-xs">
            {intentSummary}
          </Badge>
          {driftDetails?.checked_at && (
            <span className="text-xs text-muted-foreground">
              Checked {new Date(driftDetails.checked_at).toLocaleTimeString()}
            </span>
          )}
          {hasDiff && (
            <Button
      ...
      {showDiff && hasDiff && (
        <div className="border rounded-lg overflow-hidden">
          <DiffEditor
            height="40vh"
            language="yaml"
            original={intendedConfig}
            modified={configContent}
