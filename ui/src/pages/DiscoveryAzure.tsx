...
export default function DiscoveryAzurePage() {
  ...
  // Demo mode (v0.89.244): provision the credential-free demo subscription
  // and refresh the list so its row appears and can be scanned.
  ...
  const handleTryDemo = useCallback(async () => {
    ...
    setDemoError(null);
    try {
      const conn = await enableAzureDemoConnection();
        ...
        <thead className="bg-muted/40">
          <tr className="text-left">
            ...
            {/* Sampling rate analysis slice 1 chunk 3 (v0.89.124,
                ...
                between Cold-start P95 and Last seen. Azure Functions
            ...
            {/* Error rate correlation slice 1 chunk 3 (v0.89.129,
                ...
                between Sampling rate and Last seen. Azure
                Functions participates. */}
...
function RecommendationsTab({
  ...
  const onGenerate = useCallback(async () => {
    ...
    setGenError(null);
    try {
      const r = await generateAzureRecommendations(connectionID, scan);
      setRecs(r);
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">
          Draft an instrumentation plan from the latest scan of this
          subscription.
        </p>
      ...
      <AWSRecommendationsTab
        recs={recs}
        accountID={scan.subscription_id}
        scanID={scan.scan_id}
