...
// slice 4 + slice 5)" section of docs/universal-discovery-design.md.
// Slice 1 covered EC2 + Lambda; slice 2 (v0.87) added RDS; slice 3a
...
// AWS_TRUST_POLICY_ROOT_NOTE is the inline explanation rendered
// directly under the trust-policy JSON in step 2. Operators read
// `:root` as "the AWS root user" and panic; this note lives next to
...
export const awsWizard: ConnectorWizard = {
  ...
  steps: [
    ...
    },
    {
      ...
      description:
        "Squadron will run sts:AssumeRole and tiny EC2 + Lambda + RDS + S3 + ALB + EKS + DynamoDB + ECS probes to confirm the role works. No records are created until you click Save on the next step.",
