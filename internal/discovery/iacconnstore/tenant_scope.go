...
// IaC connection substrate, mirroring the proven application-store
...
// iac_connections owner-tenant column). In the OSS build this is INERT:
...
// real per-tenant isolation of the sealed IaC credentials.
//
// The HMAC-authed webhook receiver (GetByRepoFullName / GetWebhookSecret)
// carries no operator identity and calls these under
// identity.WithSystemContext, so IsSystemContext yields apply=false and
// those lookups remain unscoped — exactly the intended behavior.
...
// DefaultTenant.
var ErrTenantContextRequired = errors.New("iacconnstore: tenant-scoped operation on an unstamped context (strict tenant scoping enabled)")
