...
// OCI connection substrate, mirroring the proven application-store
...
// predicate (the Squadron owner-tenant column — DISTINCT from the OCI
// tenancy_ocid, which is a cloud-side identifier, not our tenancy). In the
// OSS build this is INERT: nothing calls SetStrictTenantScoping, so
...
// DefaultTenant.
var ErrTenantContextRequired = errors.New("ociconnstore: tenant-scoped operation on an unstamped context (strict tenant scoping enabled)")
