...
// Azure connection substrate, mirroring the proven application-store
...
// tenantScope to decide whether to add a `squadron_tenant_id = ?`
// predicate (the Squadron owner-tenant column — DISTINCT from the Azure-AD
// tenant_id, which is a cloud identifier, not our tenancy). In the OSS
// build this is INERT: nothing calls SetStrictTenantScoping, so
// strictTenantScoping stays false and an unstamped context falls back to
// identity.DefaultTenant ("default"). Since OSS writes every row with
// squadron_tenant_id='default' too, the predicate is a no-op and OSS
// behavior is byte-identical to the pre-scoping store. The enterprise wire
// flips the flag on at startup and wires a real TenantResolver, at which
// point the same predicate becomes real per-tenant isolation of the sealed
...
// DefaultTenant.
var ErrTenantContextRequired = errors.New("azureconnstore: tenant-scoped operation on an unstamped context (strict tenant scoping enabled)")

// tenantScope resolves how a scoped query should be tenanted.
// apply=false → system context: no tenant predicate (all tenants).
// apply=true  → add "squadron_tenant_id = ?" with tenant (DefaultTenant
// when unstamped and strict scoping is off; error when strict scoping is
// on).
