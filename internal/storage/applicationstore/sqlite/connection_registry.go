...
// connection_registry.go — the SQLite backend for the HA S3b connection
// registry (ADR 0035). One row per agent recording which Squadron instance
// currently owns that agent's OpAMP WebSocket.
//
// SYSTEM-SCOPED: instance identity is orthogonal to tenant, so the table has
// NO tenant_id column and these methods apply no tenant predicate. Callers
// stamp identity.WithSystemContext(ctx); nothing here reads the tenant. That
// is the deliberate difference from every per-tenant table in this package.
//
// agent_id is the Squadron fleet id (agentid.Derive) and the PRIMARY KEY: one
// owner per agent, last-writer-wins on reconnect.

// UpsertConnectionOwner records (or refreshes) this instance's ownership of
// agentID. On a fresh row connected_at = now. On a heartbeat from the SAME
// instance connected_at is preserved and only last_heartbeat_at advances. On a
// takeover by a DIFFERENT instance (reconnect landed elsewhere) connected_at is
// reset to now — a new connection. In every case instance_id + last_heartbeat_at
// are set to the incoming values, so the write is last-writer-wins on the PK.
...
// DeleteConnectionOwner removes agentID's ownership row (clean disconnect).
// Deleting a row that does not exist is a clean no-op.
func (s *Storage) DeleteConnectionOwner(ctx context.Context, agentID uuid.UUID) error {
	if _, err := s.db.ExecContext(ctx,
		`DELETE FROM connection_registry WHERE agent_id = ?`, agentID.String()); err != nil {
...
func (s *Storage) GetConnectionOwner(ctx context.Context, agentID uuid.UUID) (*types.ConnectionOwner, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT agent_id, instance_id, connected_at, last_heartbeat_at
		   FROM connection_registry WHERE agent_id = ?`, agentID.String())
...
func (s *Storage) ReclaimStaleConnectionOwners(ctx context.Context, olderThan time.Time) (int, error) {
	res, err := s.db.ExecContext(ctx,
		`DELETE FROM connection_registry WHERE last_heartbeat_at < ?`, olderThan.UTC())
	if err != nil {
