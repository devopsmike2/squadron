...
// TestWebhook_SNSKinds_RouteToAWS — event source tier slice 3 chunk
// 2. The 2 new SNS recommendation kinds
// (sns-subscriptions-attach + sns-delivery-logging-enable) must
// route through the webhook receiver as provider=aws with the
// parsed account_id surfaced on the audit payload. Table-driven to
// cover BOTH kinds — same shape as the slice 2 propagation routing
// test. Pins design doc §11 acceptance tests 10 + 11.
func TestWebhook_SNSKinds_RouteToAWS(t *testing.T) {
	for _, kind := range []string{
...
// TestProviderFromRecommendationKind_EventSourceSlice3Extension —
// pins the dispatch table for the 2 new SNS kinds and reasserts
// prior-tier routing remains green. Same shape as the orchestration
// slice 2 extension test.
func TestProviderFromRecommendationKind_EventSourceSlice3Extension(t *testing.T) {
	cases := []struct {
		kind string
		want string
	}{
		{kind: "sns-subscriptions-attach", want: "aws"},
		{kind: "sns-delivery-logging-enable", want: "aws"},
		// Prior-tier sanity — slice 1 + slice 2 event source kinds,
		// orchestration, and serverless kinds still route correctly.
		{kind: "eventbridge-xray-enable", want: "aws"},
		{kind: "eventbridge-rule-preserves-trace", want: "aws"},
		{kind: "pubsub-trace-enable", want: "gcp"},
		{kind: "servicebus-diagnostics-enable", want: "azure"},
		{kind: "streaming-logging-enable", want: "oci"},
...
// TestWebhook_SQSKinds_RouteToAWS — event source tier slice 4 chunk
// 2. The 2 new SQS recommendation kinds
// (sqs-redrive-policy-enable + sqs-deadletter-queue-attach) must
// route through the webhook receiver as provider=aws with the
// parsed account_id surfaced on the audit payload. Table-driven to
// cover BOTH kinds — same shape as the slice 3 SNS routing test.
// Pins design doc §11 acceptance tests 12 + 13.
func TestWebhook_SQSKinds_RouteToAWS(t *testing.T) {
	for _, kind := range []string{
...
// TestWebhook_CloudTasksKinds_RouteToGCP — event source tier slice 5
// chunk 2. The 2 new Cloud Tasks recommendation kinds
// (cloudtasks-retry-policy-enable + cloudtasks-logging-enable) must
// route through the webhook receiver as provider=gcp with the parsed
// project_id surfaced on the audit payload. Table-driven to cover
// BOTH kinds — same shape as the slice 4 SQS routing test.
// Pins design doc §11 acceptance tests 14 + 15.
func TestWebhook_CloudTasksKinds_RouteToGCP(t *testing.T) {
	for _, kind := range []string{
		...
		t.Run(kind, func(t *testing.T) {
			...
			branch := "squadron/rec/" + kind + "/my-sandbox-project/us-central1/abc123"
			body := makePREventBody(t, "closed", true, "octo/widgets", 42,
				branch, "2026-06-24T12:34:56Z", "alice")
			sig := signGitHubWebhook(t, body, webhookTestSecret)
			...
			if pay["project_id"] != "my-sandbox-project" {
				t.Errorf("payload.project_id = %v, want my-sandbox-project", pay["project_id"])
			}
		})
	}
}

// TestProviderFromRecommendationKind_EventSourceSlice5Extension —
// pins the dispatch table for the 2 new Cloud Tasks kinds and
// reasserts prior-tier routing remains green. Same shape as the slice
// 4 SQS extension test.
func TestProviderFromRecommendationKind_EventSourceSlice5Extension(t *testing.T) {
	cases := []struct {
		kind string
		want string
	}{
		{kind: "cloudtasks-retry-policy-enable", want: "gcp"},
		{kind: "cloudtasks-logging-enable", want: "gcp"},
		// Prior-tier sanity — slice 1-4 event source kinds, orchestration,
...
// SNS extension test.
func TestProviderFromRecommendationKind_EventSourceSlice4Extension(t *testing.T) {
	cases := []struct {
		kind string
		want string
	}{
		{kind: "sqs-redrive-policy-enable", want: "aws"},
		{kind: "sqs-deadletter-queue-attach", want: "aws"},
		// Prior-tier sanity — slice 1 + slice 2 + slice 3 event
		// source kinds, orchestration, and serverless kinds still
		// route correctly.
		{kind: "sns-subscriptions-attach", want: "aws"},
		{kind: "sns-delivery-logging-enable", want: "aws"},
		{kind: "eventbridge-xray-enable", want: "aws"},
