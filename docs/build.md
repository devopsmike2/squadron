...
| `services.SetStrictIdentitySource` (ADR 0014 Arc C) | Never called in OSS → `strictIdentitySource` stays **false**; the auth path is unchanged (a raw token authenticates as before). The slice-4d enforcement (reject a Principal whose credential lacks a validated OIDC/SCIM identity source) reads this flag but is inert until the enterprise wire flips it | `internal/services/identity_source_contract_test.go::TestStrictIdentitySource_OSSDefaultInert` |
