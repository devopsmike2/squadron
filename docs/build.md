...
| Per-tenant trace budget (ADR 0024) | `wire_tracebudget_oss.go` → `nil` (uniform global `SQUADRON_TRACEINDEX_MAX_ROWS` cap) | `wire_tracebudget_enterprise.go` → panics with guidance |
